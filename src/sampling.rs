...
use pcs::{lab_to_xyz, end_points_by_space, xyz_to_lab};
...
use {Intent, CIEXYZ, CIELab, ProfileClass, ColorSpace};
use transform::{Transform, TransformFlags};
...
fn create_roundtrip_xform(profile: &Profile, intent: Intent) -> Result<Transform, String> {
    ...
    Transform::new_extended(&profiles, &bpc, &intents, &states, None, 0, TYPE_LAB_DBL, TYPE_LAB_DBL, TransformFlags::NOCACHE | TransformFlags::NOOPTIMIZE)
}

/// Use darker colorants to obtain black point. This works in the relative colorimetric intent and
/// assumes more ink results in darker colors. No ink limit is assumed.
fn black_point_as_darker_colorant(profile: &Profile, intent: Intent, _flags: u32) -> Result<CIEXYZ, String> {
    ...
    let xform = Transform::new_flags(profile, dw_format, &h_lab, TYPE_LAB_DBL, intent, TransformFlags::NOOPTIMIZE | TransformFlags::NOCACHE)?;

    // Convert black to Lab
    let mut lab = CIELab { L: 0., a: 0., b: 0. };
    unsafe { xform.convert_any(&black as *const _ as *const (), &mut lab as *mut _ as *mut (), 1) };
...
pub fn detect_black_point(profile: &Profile, intent: Intent, dw_flags: u32) -> Result<CIEXYZ, String> {
    let dev_class = profile.device_class;

    // Make sure the device class is adequate
    if dev_class == ProfileClass::Link || dev_class == ProfileClass::Abstract || dev_class == ProfileClass::NamedColor {
        return Err("No black point for given device class".into());
    }

    // Make sure intent is adequate
    if intent != Intent::Perceptual && intent != Intent::RelativeColorimetric && intent != Intent::Saturation {
        return Err("No black point for given intent".into());
    }

    // v4 + perceptual & saturation intents does have its own black point, and it is
    // well specified enough to use it. Black point tag is deprecated in V4.
    if profile.encoded_version() > 0x4000000 && (intent == Intent::Perceptual || intent == Intent::Saturation) {
    ...
    if intent == Intent::RelativeColorimetric && dev_class == ProfileClass::Output && profile.color_space == ColorSpace::CMYK {
...
pub fn detect_dest_black_point(profile: &Profile, intent: Intent, dw_flags: u32) -> Result<CIEXYZ, String> {
    let dev_class = profile.device_class;
    ...
    if !profile.is_clut(intent, USED_AS_OUTPUT) || (color_space != ColorSpace::Gray && color_space != ColorSpace::RGB && color_space != ColorSpace::CMYK) {
    ...
    for l in 0..256 {
        ...
        let mut dest_lab = CIELab { L: 0., a: 0., b: 0. };
        unsafe { round_trip.convert_any(&lab as *const _ as *const (), &mut dest_lab as *mut _ as *mut (), 1) };
    ...
    if intent == Intent::RelativeColorimetric {
        for l in 0..256 {
            if !(in_ramp[l] <= min_l + 0.2 * (max_l - min_l) || (in_ramp[l] - out_ramp[l]).abs() < 4.) {
